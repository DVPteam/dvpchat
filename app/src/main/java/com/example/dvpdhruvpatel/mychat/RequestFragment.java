...
public class RequestFragment extends Fragment
{
    ...
    private DatabaseReference friendsRequestReferances;
                             ...
                             Bundle savedInstanceState)
    {
        ...
        friendsRequestReferances = (DatabaseReference) FirebaseDatabase.getInstance().getReference().child("Friends_Requests").child(online_user_id);
                ...
                final String list_user_id = getRef(position).getKey();
                 ...
                 get_type_ref.addValueEventListener(new ValueEventListener()
                 {
                     ...
                     public void onDataChange(DataSnapshot dataSnapshot)
                     {
                         ...
                         {
                             String request_tpye = dataSnapshot.getValue().toString();

                             if (request_tpye.equals("received"))
                             {
                                 userreferance.child(list_user_id).addValueEventListener(new ValueEventListener() {
                                     @Override
                                     public void onDataChange(DataSnapshot dataSnapshot)
                                     {
                                                 ...
                                                 builder.setItems(options, new DialogInterface.OnClickListener()
                                                 {
                                                     ...
                                                     public void onClick(DialogInterface dialogInterface, int position)
                                                     {
                                                         if (position == 0)
                                                         {
                                                             ...
                                                             FriendsDatabaseRef.child(online_user_id).child(list_user_id).child("date").setValue(save_current_date)
                                                                     .addOnCompleteListener(new OnCompleteListener<Void>()
                                                                     {
                                                                         @Override
                                                                         public void onComplete(@NonNull Task<Void> task)
                                                                         {
                                                                             FriendsDatabaseRef.child(list_user_id).child(online_user_id).child("date").setValue(save_current_date).addOnSuccessListener(new OnSuccessListener<Void>()
                                                                             {
                                                                                 @Override
                             ...
                             else if (request_tpye.equals("sent"))
                             {
                                 Button req_sent_btn = viewHolder.mView.findViewById(R.id.request_accpet_btn);
